    ...
    setStockStatus: function(inStock) {
        if (inStock) {
            this._E.availability.each(function(el) {
                var el = $(el),
                    circle = '<i class="fa fa-check-circle-custom" aria-hidden="true"></i>';
                    $circle = new Element('div').update(circle).down('i');
                el.addClassName('in-stock').removeClassName('out-of-stock');
                el.select('span').invoke('update', Translator.translate(' Available'));
                el.select('span')[0].insert({'top':$circle});
            });
