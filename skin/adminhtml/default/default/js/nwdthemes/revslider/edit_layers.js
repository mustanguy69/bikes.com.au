...
UniteLayersRev = new function(){
	...
	********************************/
	...
	var initButtons = function(){
		...
		jQuery("#button_add_product_layer_image").click(function(){
			if(jQuery(this).data('isstatic') == true)
			{
				addProductLayerImage('static');
			}
			else
			{
				addProductLayerImage();
			}
		});
...
/*					stop: function() {
	...
	/**
	 * add product image layer
	 */
	var addProductLayerImage = function(special_type){

		objLayer = {
			style : "",
			text : "Product Image " + (id_counter+1),
			type : "image",
			image_url : g_productImagePlaceholder,
			product_image: true
		};
	...
	**/
	...
	t.updateLayerFromFields = function(){
		...
		// fix for incorrect layer type for image
		var _objLayer = getLayer(selectedLayerSerial);
		if(_objLayer.type)
			type = _objLayer.type;
	...
	var setLayerSelected = function(serial){
		...
		switch(objLayer.type){
			...
			case "image":
				...
				if (typeof objLayer.product_image == 'undefined' || ! objLayer.product_image)
				{
