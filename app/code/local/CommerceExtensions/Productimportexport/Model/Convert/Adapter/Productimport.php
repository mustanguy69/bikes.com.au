...
{
	static $image_file_name_cache = array();
	/**
     * Affected entity ids
     *
     * @var array
	...
	*/
	public function saveRow( array $importData )
	{
		...
		if($this->getBatchParams('update_products_only') == "true") {
			$productIdupdate = $product->getIdBySku($importData['sku']);
			if ($productIdupdate) {
				$product->load($productIdupdate);
			} else {
		...
		$finalsuperattributetype = $importData['prodtype'];
		...
		if ($productId) {
             ...
             */
            if (empty($importData['prodtype']) || !isset($productTypes[strtolower($importData['prodtype'])])) {
                $value = isset($importData['prodtype']) ? $importData['prodtype'] : '';
                $message = Mage::helper('catalog')->__('Skip import row, is not valid value "%s" for field "%s"', $value, 'prodtype');
                Mage::throwException($message);
				Mage::log(sprintf('Skip import row, is not valid value "'.$value.'" for field prodtype', $message), null,'ce_product_import_export_errors.log');
            }
            $product->setTypeId($productTypes[strtolower($importData['prodtype'])]);
            /**
		...
		$currentproducttype = $importData['prodtype'];
		
		if ($importData['prodtype'] == 'configurable') {
			...
			if (!empty($usingAttributeIds)) {
				...
				$updateconfigurablearray = $product->getTypeInstance()->getConfigurableAttributesAsArray();
					...
					foreach($updateconfigurablearray as $eacharrayvalue) {	
					 if($this->getBatchParams('configurable_use_default') > 0) {			
		...
		if ($importData['prodtype'] == 'downloadable' && $importData['downloadable_options'] != "") {
			if ($new) {
				...
				foreach ($commadelimiteddata as $data) {
					...
					if ($importData['prodtype'] == 'downloadable' && $importData['downloadable_sample_options'] != "") {
						$samplecommadelimiteddata = explode('|',$importData['downloadable_sample_options']);
						foreach ($samplecommadelimiteddata as $sample_data) {
							$downloadable_sample_options_data = $this->userCSVDataAsArray($sample_data);
							
							$downloadableitems['sample'][$sampledownloadableitemsoptionscount]['is_delete'] = 0;
							$downloadableitems['sample'][$sampledownloadableitemsoptionscount]['sample_id'] = 0;
							...
							if($downloadable_sample_options_data[1] == "file") {
								$downloadableitems['sample'][$sampledownloadableitemsoptionscount]['sample_file'] = $downloadable_sample_options_data[2];
							
							} else if($downloadable_sample_options_data[1] == "url") {
								$downloadableitems['sample'][$sampledownloadableitemsoptionscount]['sample_url'] = $downloadable_sample_options_data[2];
								
							}
							#$product->setDownloadableData($sampledownloadableitems);
				...
				#print_r($downloadableitems);
				  ...
				  $download_info->setProduct($product);
					...
					if ($importData['prodtype'] == 'downloadable' && $importData['downloadable_sample_options'] != "") {
						if ($download_info->hasSamples()) {
							$_sample_links=$download_info->getSamples();
		...
		if ($importData['prodtype'] == 'bundle') {
			if ($new) {
				...
				foreach ($commadelimiteddata as $data) {
					...
					if($importData['bundle_selections'] !="") {
		...
		*/
		...
		foreach ( $this -> _ignoreFields as $field ) {
			// May need to do something like this where people made a "type" attribute as its still considered a core one by magento core
			//if ( isset( $importData[$field] ) && $fields != "type") {
		...
		if ( isset( $importData['websites'] ) ) {
			if($this->getBatchParams('append_websites') == "true") { 
				$websiteIds = $product -> getWebsiteIds();
				if ( !is_array( $websiteIds ) ) {
					$websiteIds = array();
				} 
...
/* END CUSTOM OPTION CODE */
			...
			if($this->getBatchParams('import_attribute_value') == "true") {
				if($this->getBatchParams('attribute_for_import_value') != "") {
					$attributestocheck = explode(',', trim($this->getBatchParams('attribute_for_import_value')));
					foreach ($attributestocheck as $single_attribute) {    
						if ( $field == $single_attribute ) {
							$value2=array();
							$attribute_model        = Mage::getModel('eav/entity_attribute');
							$attribute_options_model= Mage::getModel('eav/entity_attribute_source_table') ;
							$attribute_code         = $attribute_model->getIdByCode('catalog_product', $field);
							$attribute              = $attribute_model->load($attribute_code);
							$attribute_table        = $attribute_options_model->setAttribute($attribute);
							$options                = $attribute_options_model->getAllOptions(false);
								
							$optionsFromImport = array();
							$optionsFromDatabase = array();
							$optionsToAdd = array();
							
							if (count($options) > 0){
								foreach($options as $option)
								{	
									$optionsFromDatabase[] = $option['label'];
									$optionsFromDatabase = array_unique($optionsFromDatabase);
									$productModel2 = Mage::getModel('catalog/product')->loadByAttribute('sku',$importData['sku']); 
									if($importData[$field] !="") {
										//checks if it's multi-select
										if (strpos($importData[$field],' , ') !== false) {
											$attributestocheckvalues = explode(' , ', $importData[$field]);
											foreach ($attributestocheckvalues as $single_attribute_value_for_import) {   
												$optionsFromImport[] = trim($single_attribute_value_for_import);
											}
										} else {
											$optionsFromImport[] = $importData[$field];
										}
										$optionsFromImport = array_unique($optionsFromImport);
									}
								}
								$optionsToAdd  = array_diff($optionsFromImport, $optionsFromDatabase);
								$optionsToAdd = array_merge($optionsToAdd); //reset Keys
							
							} else { // attribute exist but no values
								...
								$optionsToAdd  = $optionsFromImport;
							}
							
							
							foreach($optionsToAdd as $k => $optionToAdd)
							{	
								$value2['option_'.$k][] = $optionToAdd;
							}
		
							$result = array('value' => $value2);
			...
			if($product->getStoreId() == "0") {	
				...
				$_product = Mage::getModel('catalog/product')->setStoreId(0)->load($product->getId());
				
				if ($attribute -> getFrontendInput() == 'boolean') {
					$currentAttrValue = $_product->getResource()->getAttribute($field)->getFrontend()->getValue($product);
				} else {
					$currentAttrValue = $_product->getData($field);
				}
				
				if($currentAttrValue == $setValue && !in_array($field, $arrayOfFieldstoSkip)) {
		...
		if ( !$product -> getVisibility() && $product->getStoreId() == "0" ) {
		...
		foreach ( $inventoryFields as $field ) {
			if ( isset( $importData[$field] ) ) {
				if ( in_array( $field, $this -> _toNumber ) ) {
					...
					// QTY INCREMENT OPTION
					//<var name="qty_increments"><![CDATA[false]]></var>
					/*
					if($field == "qty" && $this->getBatchParams('qty_increments') == "true") {
						if ($stockItem = $product->getStockItem()) {
							#echo "QTY: " . $this->getNumber($stockItem->getData('qty')) . "<br/>";
							$original_qty = $this->getNumber($stockItem->getData('qty'));
							$final_qty = $this->getNumber($importData[$field]) + $original_qty;
							$stockData[$field] = $final_qty;
						} else {
							$stockData[$field] = $this->getNumber($importData[$field]);
						}
					} else {						
		 ...
		 if($importData["image"] != "" || $importData["small_image"] != "" || $importData["thumbnail"] != "" ) {		
			...
			foreach ($product->getMediaAttributes() as $mediaAttributeCode => $mediaAttribute) {
				if (isset($importData[$mediaAttributeCode])) {
					...
					//$posImage = strpos($importData[$mediaAttributeCode], "+");
					//if ($posImage !== false) { }
					
					if($this->getBatchParams('import_images_by_url') == "true" && $file != "" && $file != "no_selection") { 
						$orgfile = $importData[$mediaAttributeCode];
						$fields = $mediaAttributeCode;
						$path_parts = pathinfo($orgfile);
						$file = DS . "image_" . $path_parts['basename'];
						$fullpath = Mage::getBaseDir('media') . DS . 'import' . $file;
						try {
							#echo "URL: " . $orgfile . "FINALNAME: " . $file;
			...
			if($this->getBatchParams('multi_store_images') == "true") { 
				// if the image file allready was imported with an previous product, dont import it twice only link to the allready imported file
				foreach ($arrayToMassAdd AS $mediaKey => $mediaValue) {
					#print_r(self::$image_file_name_cache);
					if (!empty(self::$image_file_name_cache[$mediaValue['file']])) {
					#if (array_key_exists($mediaValue['file'], $image_file_name_cache)) {
						$mediaGalleryBackendModel->setMediaAttribute($product, $mediaValue['mediaAttribute'], self::$image_file_name_cache[$mediaValue['file']]);
						unset($arrayToMassAdd[$mediaKey]);
					}
				}
			}
			...
			if($this->getBatchParams('multi_store_images') == "true") { 
				...
				if (!empty($addedFilesCorrespondence['alreadyAddedFiles'])) {
					foreach ($addedFilesCorrespondence['alreadyAddedFiles'] AS $file_key => $import_file_name) {
						$final_file_name = $addedFilesCorrespondence['alreadyAddedFiles'][$file_key];
						self::$image_file_name_cache[$import_file_name] = $final_file_name;
						#print_r(self::$image_file_name_cache);
					}
				}
			}
			...
			if ( !empty( $importData['gallery'] ) ) {
				...
				$countforgallery=0;
				if (isset($importData['gallery_label'])) {
					if (!empty( $importData['gallery_label'])) {
						//starts custom gallery label code
						$gallerylabelData = explode(';',$importData["gallery_label"]);
					}	
				}
...
if(trim($gallery_img) != $importData["image"]) 
{
	if(trim($gallery_img) != $importData["small_image"]) 
	{
		if( trim($gallery_img) != $importData["thumbnail"]) 
		{
						//
						if($this->getBatchParams('import_images_by_url') == "true") { 
							
							$fileurlpath = $gallery_img;
							$path_parts = pathinfo($fileurlpath);
							$gallery_img = DS . "image_" . $path_parts['basename'];
							$fullpath = Mage::getBaseDir('media') . DS . 'import' . $gallery_img;
							try {
							#echo "URL: " . $fileurlpath . "FINALNAME: " . $gallery_img;
						...
						*/
						if($this->getBatchParams('exclude_gallery_images') == "true") {
						  if(!empty($gallerylabelData[$countforgallery])) {
						  $product -> addImageToMediaGallery( Mage :: getBaseDir( 'media' ) . DS . 'import' . $gallery_img, null, false, true, $gallerylabelData[$countforgallery]);
						  } else {
						  $product -> addImageToMediaGallery( Mage :: getBaseDir( 'media' ) . DS . 'import' . $gallery_img, null, false, true );
						  }
						} else {
						  if(!empty($gallerylabelData[$countforgallery])) {
						  $product -> addImageToMediaGallery( Mage :: getBaseDir( 'media' ) . DS . 'import' . $gallery_img, null, false, false, $gallerylabelData[$countforgallery]);
						  } else {
						  $product -> addImageToMediaGallery( Mage :: getBaseDir( 'media' ) . DS . 'import' . $gallery_img, null, false, false );
						  }
						}
		...
		try {
			...
			Mage::log(sprintf('failed to import sku: ' . $importData["sku"] .' %s', $e->getMessage()), null,'ce_product_import_export_errors.log');
		}
		...
		if($this->getBatchParams('import_attribute_value') == "true") {
			if($this->getBatchParams('attribute_for_import_value') != "") {
				...
				foreach ($attributestocheck as $single_attribute) {   
					if ($single_attribute != "") {
						#if($new) {
							$fixnewAddedAttribute = Mage::getModel('catalog/product')->load($product->getId()); 
							$setValue = $importData[$single_attribute];
							$finalattributevalueforimport = "";
							$resource = Mage::getSingleton('core/resource');
							$prefix = Mage::getConfig()->getNode('global/resources/db/table_prefix');
							$read = $resource->getConnection('core_read');
							$select_attribute_options_qry = "SELECT ".$prefix."eav_attribute.*, ".$prefix."eav_attribute_option_value.* FROM `".$prefix."eav_attribute` INNER JOIN ".$prefix."eav_attribute_option ON ".$prefix."eav_attribute_option.attribute_id = ".$prefix."eav_attribute.attribute_id INNER JOIN ".$prefix."eav_attribute_option_value ON ".$prefix."eav_attribute_option_value.option_id = ".$prefix."eav_attribute_option.option_id WHERE ".$prefix."eav_attribute.attribute_code = '".$single_attribute."'";
							
							//checks if it's multi-select
							if (strpos($importData[$single_attribute],' , ') !== false) {
								
								$attributestocheckvalues = explode(' , ', $importData[$single_attribute]);
								foreach ($attributestocheckvalues as $single_attribute_value_for_import) {  
								
								$attributeoptionrows = $read->fetchAll($select_attribute_options_qry);
								foreach($attributeoptionrows as $attributeoptiondata)
								 {
									 if($single_attribute_value_for_import == $attributeoptiondata["value"]) {
										$finalattributevalueforimport .= $attributeoptiondata['option_id'] . ",";
									 }
								 }
								}
								$cleaned_finalattributevalueforimport = substr_replace($finalattributevalueforimport,"",-1);
								$fixnewAddedAttribute -> setData($single_attribute, array($cleaned_finalattributevalueforimport));
								$fixnewAddedAttribute -> save();
							} else { 
								$attributeoptionrows = $read->fetchAll($select_attribute_options_qry);
								 foreach($attributeoptionrows as $attributeoptiondata)
								 {
									 if($setValue == $attributeoptiondata["value"]) {
										$finalattributevalueforimport = $attributeoptiondata['option_id'];
									 }
								 }
								$fixnewAddedAttribute -> setData($single_attribute, $finalattributevalueforimport);
								$fixnewAddedAttribute -> save();
						   }
						#}
					}
		...
		//END EXTRA FIX
		//WEE TAX RULES
		if( isset($importData['weee']) && !empty($importData['weee']) ) {
			//$this->_editWeee($product, $importData['weee'], $store);
		}
		...
		if ( isset( $importData['product_tags'] ) && $importData['product_tags'] !="" ) {
			...
			if(!empty($configProductTags)) {
				foreach ($configProductTags as $tagName) {
					...
					if(isset($commadelimiteddata[1])) { $tagName = $commadelimiteddata[1]; }
					$productIds = array();
					...
					if(isset($commadelimiteddata[0]) && isset($commadelimiteddata[1])) {
						if($commadelimiteddata[0] != "") {
							...
							$model->unsetData()->loadByName($tagName);
					
							$data['name']               = trim($tagName);
							$data['status']             = $model->getApprovedStatus();
...
if ($finalsuperattributepricing != "") {
					...
					//might need this website_id.. and mod sql below to accept website_id for multi-store views
...
if ($finalsuperattributetype == 'grouped' && !empty($finalIDssthatneedtobeconvertedto)) {
		...
		foreach($finalIDssthatneedtobeconvertedto as $data_product_position_id)
		{
			 ...
			 if($groupedpositionproducts == true) {
				 $select_qry_catalog_product_link = "SELECT link_id FROM ".$prefix."catalog_product_link 
																WHERE product_id = '" . $product->getId() . "' 
																AND linked_product_id = '" . trim($data_productID) . "' 
			...
			if($finalgroup_price_price != "delete") {
				...
				foreach($group_price_priceData as $singleattributeData) {
					...
					if($FinalGroupedPriceData[0] != "") {
						if($new) {
							#echo "SQL2: ";
							$insertPrice='INSERT into '.$resource->getTableName('catalog/product_attribute_group_price').' (entity_id,all_groups,customer_group_id,value,website_id) VALUES ("'.$product->getId().'","0","'.$FinalGroupedPriceData[0].'","'.$FinalGroupedPriceData[1].'","'.$websiteInternalId.'");';
							$adapter->query($insertPrice);
							
						} else {
							#echo "SQL UPDATE: " . $updatePrice;
							$select_qry2 = $read->query("SELECT value_id FROM ".$resource->getTableName('catalog/product_attribute_group_price')." WHERE value = '".$FinalGroupedPriceData[1]."' AND customer_group_id = '".$FinalGroupedPriceData[0]."' AND website_id = '".$websiteInternalId."' AND entity_id = '".$product->getId()."'");
...
/* END FIX FOR IMAGE LABLES */
	...
	private function _editWeee(&$product, $weee_field = false, $store)
	{
		if (($weee_field) && !empty($weee_field)) {
            
            if(trim($weee_field) == 'REMOVE'){
            
                $product->setData('weee', array());
            
            } else {
                
                
                if($this->getBatchParams('append_weee') == "true") { 
               		 //get current product tier prices
                	$existing_weee = $product->getData('weee');
				} else {
                	$existing_weee = array();
				}
                ...
                foreach($existing_wee as $key => $etp){
                    $weee_lookup[intval($etp['value'])] = $key;
                }
                
                //parse incoming tier prices string
				#print_r($weee_field);
                $incoming_weee = explode('|',$weee_field);
				$tps_toAdd = array();  
				$tierpricecount=0;              
				foreach($incoming_weee as $weee_str){
					#echo "t: " . $weee_str;
                    if (empty($weee_str)) continue;
                    
                    $tmp = array();
                    $tmp = explode('=',$weee_str);
                    
					#echo ('adding weee');
                    #print_r($tmp);
                    $tps_toAdd[$tierpricecount] = array(
                                        'website_id' => $tmp[0], // !!! so is this
                                        'country' => $tmp[1],
                                        'state' => $tmp[2],
                                        'value' => $tmp[3]
                                    );
                    ...
                    if(isset($weee_lookup[intval($tmp[1])])){
                        unset($existing_weee[$weee_lookup[intval($tmp[1])]]);
                    }
                    $tierpricecount++;
                }

                //combine array
                $tps_toAdd = array_merge($existing_weee, $tps_toAdd);
               
				#print_r($tps_toAdd);
                //save it
                #$product->setData('weee', $tps_toAdd);
				...
				foreach( $tps_toAdd as $val ) { 
								
					if($new) {
						$select_qry3 = $read->query("SELECT region_id FROM ".$prefix."directory_country_region WHERE code = '".$val['state']."' AND country_id = '".$val['country']."'");
						$newrowItemId2 = $select_qry3->fetch();
						$region_id = $newrowItemId2['region_id'];
						if($region_id == "") {
							$region_id = "*";
						}
						$select_qry3 = $read->query("SELECT attribute_id FROM ".$prefix."eav_attribute AS eav WHERE eav.attribute_code = 'weee' and eav.entity_type_id ='".$EntityTypeId."'");
						$newrowItemId3 = $select_qry3->fetch();
						$attribute_id = $newrowItemId3['attribute_id'];
							
						$insertPrice='INSERT into '.$prefix.'weee_tax (website_id, entity_id, country, value, state, attribute_id, entity_type_id) VALUES ("'.$val['website_id'].'", "'.$product->getId().'", "'.$val['country'].'", "'.$val['value'].'", "'.$region_id.'", "'.$attribute_id.'", "'.$EntityTypeId.'");';
						#echo "SQL2: " . $insertPrice;
						$adapter->query($insertPrice);
					} else {
						if($val['website_id'] != "") {
						
							$select_qry2 = $read->query("SELECT value_id FROM ".$prefix."weee_tax WHERE value = '".$val['value']."' AND entity_id = '".$product->getId()."'");
							$newrowItemId2 = $select_qry2->fetch();
							$db_product_id = $newrowItemId2['value_id'];
							if($db_product_id == "") {
							...
							$updatePrice="UPDATE ".$prefix."weee_tax SET value = '".$val['value']."' WHERE value_id = '".$db_product_id."'";
							#echo "SQL UPDATE: " . $updatePrice;
							$adapter->query($updatePrice);
							}
						}
	...
	private function _editTierPrices(&$product, $tier_prices_field = false, $store)
	{
		...
		if (($tier_prices_field) && !empty($tier_prices_field)) {
            ...
            if(trim($tier_prices_field) == 'REMOVE'){
                ...
                if($this->getBatchParams('append_tier_prices') != "true") {
					 $productId = $product->getId();
					 $delete_qry =$read->query("DELETE FROM `".$prefix."catalog_product_entity_tier_price` WHERE `entity_id` = '".$productId."' AND website_id = '".$store->getWebsiteId()."'");
                	 $existing_tps = array();
				...
				$this->_setTierPricing($tps_toAdd, $qty_format_number, $productId);
				           
            }
        }
	}

	public function _setTierPricing($tps_toAddArray, $qty_format_number, $productId) {
	
		$read = Mage::getSingleton('core/resource')->getConnection('core_read');
		$prefix = Mage::getConfig()->getNode('global/resources/db/table_prefix');
		$write = Mage::getSingleton('core/resource')->getConnection('core_write');
		
		foreach($tps_toAddArray as $tps_toAdd){
				
			 if($tps_toAdd['cust_group'] == "32000") {
				
				 $select_qry =$read->query("select value_id from `".$prefix."catalog_product_entity_tier_price` where `entity_id` = $productId AND `qty` = '".$tps_toAdd['price_qty']."' AND `all_groups` = 1 AND `website_id` = '".$tps_toAdd['website_id']."'");
				 ...
				 if($newGroupId != "") {
					...
					$write->query("
					  ...
					  SET  val.value = '".$tps_toAdd['price']."', val.qty = '".$tps_toAdd['price_qty']."'
					  WHERE val.value_id = '".$newGroupId."'
					");
					
				} else {
					#echo "ID2: " . $newGroupId;
					$write_qry =$write->query("Insert into `".$prefix."catalog_product_entity_tier_price` (entity_id,all_groups,customer_group_id,qty,value,website_id) VALUES ('$productId','1','0','".$tps_toAdd['price_qty']."','".$tps_toAdd['price']."','".$tps_toAdd['website_id']."')");
				
				}
				
			 } else {
				
				 $select_qry =$read->query("select value_id from `".$prefix."catalog_product_entity_tier_price` where `entity_id` = $productId AND `qty` = '".$tps_toAdd['price_qty']."' AND `customer_group_id` = '".$tps_toAdd['cust_group']."' AND `website_id` = '".$tps_toAdd['website_id']."'");
				 //echo "select value_id from `".$prefix."catalog_product_entity_tier_price` where `entity_id` = $productId AND `qty` = '".$qty_format_number."'  AND `customer_group_id` = $tmp[0]";
				 $newrow = $select_qry->fetch();
				 $newGroupId = $newrow['value_id'];
				 if($newGroupId != "") {
					...
					$write->query("
					  UPDATE ".$prefix."catalog_product_entity_tier_price val
					  SET  val.value = '".$tps_toAdd['price']."', val.qty = '".$tps_toAdd['price_qty']."', val.customer_group_id = '".$tps_toAdd['cust_group']."'
					  WHERE val.value_id = '".$newGroupId."'
					");
				} else {
					#echo "ID2: " . $newGroupId;
					#echo "sql" . "Insert into `".$prefix."catalog_product_entity_tier_price` (entity_id,all_groups,customer_group_id,qty,value,website_id) VALUES ('$productId','0','".$tmp[0]."','".$tmp[1]."','".$tmp[2]."','".$store->getWebsiteId()."')";
					$write_qry =$write->query("Insert into `".$prefix."catalog_product_entity_tier_price` (entity_id,all_groups,customer_group_id,qty,value,website_id) VALUES ('$productId','0','".$tps_toAdd['cust_group']."','".$tps_toAdd['price_qty']."','".$tps_toAdd['price']."','".$tps_toAdd['website_id']."')");
				
				}
				
			 } //ends check on if customer is part of all groups2
