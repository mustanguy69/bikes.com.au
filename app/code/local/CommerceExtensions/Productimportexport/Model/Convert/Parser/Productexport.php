...
/**
	...
	public function exportProductData($product, $export_multi_store, $i, $read, $prefix, $websiteCode="")
    {
				...
				if ($this->getStore()->getCode() == Mage_Core_Model_Store::ADMIN_CODE) {
					...
					foreach ($product->getWebsiteIds() as $websiteId) {
						...
						try {
							...
							$this->addException(Mage::helper('catalog')->__('Try resaving following product ID ['.$product->getId().']: '), Varien_Convert_Exception::FATAL);
							throw $e;
						}
            ...
            foreach ($product->getData() as $field => $value) {
				...
				if($this->getVar('export_full_image_paths') == "true" && $field == "image" || $this->getVar('export_full_image_paths') == "true" && $field == "small_image" || $this->getVar('export_full_image_paths') == "true" && $field == "thumbnail") {
					if($value !="no_selection" && $value !="") { $row[$field] = Mage::getBaseUrl('web') . "media/catalog/product" . $value; }else{$row[$field] = $value;}
				} else {
					$row[$field] = $value;
				}
            }
					 ...
					 /* ADDITIONAL CATEGORY ID EXPORT FOR 1.4 ONLY [END] */
						...
						if (count($galleryImagesModel['images']) > 0) {
							foreach ($galleryImagesModel['images'] as $_image) {
								...
								if($this->getVar('export_full_image_paths') == "true" && $_image['file'] !="") {
						...
						/*
						$incoming_weee = $product->getData('weee');
						if(is_array($incoming_weee)) {
							foreach($incoming_weee as $weee_str){
								#print_r($tier_str);
								$finalweee .= $weee_str['website_id'] . "=" . $weee_str['country'] . "=" . $weee_str['state'] . "=" . $weee_str['value'] . "|";
							}
						}
						$row['weee'] = substr_replace($finalweee,"",-1);
						*/
						/* EXPORT WEEE TAX */
						...
						if($product->getTypeId() == "configurable") {
							...
							if($product->getTypeId() == "bundle") {
								...
								foreach($optionModel as $eachOption) {
										...
										//$selectionModel = $productbundleItem->getTypeInstance(true)->getSelectionsCollection($productbundleItem->getTypeInstance(true)->getOptionsIds($productbundleItem), $productbundleItem);
    ...
    public function unparse()
    {
	    ...
	    if($this->getVar('export_filter_by_category_ids') != "") {	
		  
		  $intCount=0;
		  $storeId = Mage::app()->getStore()->getId();  
		  $resource = Mage::getSingleton('core/resource');
		  $read1 = $resource->getConnection('catalog_read');
		  $categoryProductTable = $resource->getTableName('catalog/category_product');
		  $productEntityIntTable = (string)Mage::getConfig()->getTablePrefix() . 'catalog_product_entity_int';
		  $arrofcats = explode(',', $this->getVar('export_filter_by_category_ids'));
          foreach ($arrofcats as $allowedcategoryID) {
		 
		    $categoryId = $allowedcategoryID;
			if ($categoryId > 0){
				$select = $read1->select()
						 ->from(array('cp'=>$categoryProductTable))
						 ->where('cp.category_id=?', trim($categoryId));
			}
			
             $productByCategories = $read1->fetchAll($select);;
		  }
		  //clean unneeded data
          foreach ($productByCategories as $_val) {
		 	unset($productByCategories[$intCount]['category_id']);
		 	unset($productByCategories[$intCount]['position']);
			$intCount++;
		  }
		  
		  $entityIds_cat_filters = Mage::getModel('catalog/product')->getCollection()
							->addAttributeToSelect('*')
							->addAttributeToFilter('entity_id', array('in' => $productByCategories));
							
		  if($this->getVar('filter/type')!="") {
			$entityIds_cat_filters->addAttributeToFilter('type_id', array('eq' => $this->getVar('filter/type')));
		  }
		  if($this->getVar('filter/status')!="") {
			$entityIds_cat_filters->addAttributeToFilter('status', array('eq' => $this->getVar('filter/status')));
		  }
		  if($this->getVar('filter/visibility')!="") {
			$entityIds_cat_filters->addAttributeToFilter('visibility', array('eq' => $this->getVar('filter/visibility')));
		  }
		  ...
		  $entityIds_cat_filters->addAttributeToSort('type_id', 'DESC')->load();	
		  $entityIds = $entityIds_cat_filters;
		  #print_r($entityIds);
		  #exit; 
		} else if($this->getVar('export_filter_by_manufacturer') != "") {	
