...
class CommerceExtensions_Advancedcustomerimportexport_Model_Convert_Adapter_Customerimport
    extends Mage_Eav_Model_Convert_Adapter_Entity
{
    ...
    public function getLastCustomerId() {
        //$this->customerRegion = $customerId;
        $LastCustomerId = Mage::getSingleton('core/session')->getLastCustomerId();
        return $LastCustomerId;
    }
    public function setLastCustomerId($customerId) {
        //$this->customerRegion = $customerId;
        Mage::getSingleton('core/session')->setLastCustomerId($customerId);
    }
    ...
    public function __construct()
    {
		...
		if (!in_array("vat_id", $this->_billingFields)) {
            $this->_billingFields[] = 'billing_vat_id';
		}
		if (!in_array("vat_id", $this->_shippingFields)) {
            $this->_shippingFields[] = 'shipping_vat_id';
		}
		/*if (in_array("vat_id", $this->_addressFields[])) {
            $this->_addressFields[] = 'vat_id';
		}*/
    }
...
//                        }
    ...
    public function saveRow($importData)
    {
		...
		if($this->getBatchParams('insert_customer_id') == "true") { 
			if (!empty($importData['customer_id'])) {
			
				$resource = Mage::getSingleton('core/resource');
				$prefix = Mage::getConfig()->getNode('global/resources/db/table_prefix');
				$write = $resource->getConnection('core_write');
		
				$finalcustomerID = $importData['customer_id'];
				#echo "ALTER TABLE ".$prefix."customer_entity AUTO_INCREMENT = ".$finalcustomerID."";
				$write->query("ALTER TABLE ".$prefix."customer_entity AUTO_INCREMENT = ".$finalcustomerID."");
			}
		}
		...
		if($this->getBatchParams('auto_create_customer_groups') == "true") { 
					
			$code = isset($importData['group']) ? $importData['group'] : '';
        ...
        if (!$customer->getId()) {
			...
			if (isset($importData['password'])) {
				#echo "HERE: " . $importData['password'];
				$cleartextpassword = trim($importData['password']);
                $customer->setPassword($cleartextpassword);
		...
		if($this->getBatchParams('update_customer_password') == "true") { 
			if (isset($importData['password'])) {
				#echo "HERE: " . $importData['password'];
				$cleartextpassword = trim($importData['password']);
                $customer->setPassword($cleartextpassword);
        ...
        if ($importBillingAddress) {
            ...
            if ($customer->getDefaultBilling()) {
				if($this->getBatchParams('import_multiple_customer_address') == "false") {
            ...
            if ($customer->getId() && $this->getLastCustomerId() != $customer->getId()) {
		...
		if(isset($importData['customer_since'])) {
			$importDate = date("Y-m-d H:i:s", Mage::getModel('core/date')->timestamp($importData['customer_since']));
		...
		if($this->getBatchParams('email_customer_password') == "true") { 
        ...
        if ($importBillingAddress && !$savedBillingAddress) {
            ...
            $billingAddress->setCustomerId($customer->getId());
            $billingAddress->save();
            if($this->getLastCustomerId() != $customer->getId()) {
                $customer->setDefaultBilling($billingAddress->getId());
            }
            if ($onlyAddress) {
                $customer->setDefaultShipping($billingAddress->getId());
        ...
        $this->setLastCustomerId($customer->getId());
