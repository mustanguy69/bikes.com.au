...
class CommerceExtensions_Advancedcustomerimportexport_Model_Convert_Parser_Customerexport
    ...
    public function unparse()
    {
        ...
        foreach ($entityIds as $i => $entityId) {
            ...
            if (!$defaultBillingId) {
				...
				if($customerAddress->getDataUsingMethod("vat_id") != "") {
					$row['billing_vat_id'] = $customerAddress->getDataUsingMethod("vat_id");
				} else {
					$row['billing_vat_id'] = null;
				}
            }
            if (!$defaultShippingId) {
				...
				if($customerAddress->getDataUsingMethod("vat_id") != "") {
					$row['shipping_vat_id'] = $customerAddress->getDataUsingMethod("vat_id");
				} else {
					$row['shipping_vat_id'] = null;
				}
            }
			...
			$row['customer_since'] = $customer->getCreatedAtTimestamp();
			...
			if($this->getVar('export_customer_id') == "true") {
            	$row['customer_id'] = $customer->getId();
			}
			
			if($this->getVar('export_multiple_addresses') == "true") {
			
				$alreadyran = false;
				$customerModel = Mage::getModel('customer/customer')->load($customer->getId());
				$allcustomeraddress = $customerModel->getAddresses();
				$itemcountfull=count($allcustomeraddress); 
				
				foreach ($allcustomeraddress as $address) {
					#Zend_Debug::dump($address->debug());
					$row['billing_firstname'] = $address->getData('firstname');
					$row['billing_lastname'] = $address->getData('lastname');
					$row['billing_street_full'] = $address->getData('street');
					$row['billing_street1'] = $address->getStreet(1);
					$row['billing_street2'] = $address->getStreet(2);
					$row['billing_city'] = $address->getData('city');
					$row['billing_region'] = $address->getData('region');
					$row['billing_country'] = $address->getData('country_id');
					$row['billing_postcode'] = $address->getData('postcode');
					$row['billing_telephone'] = $address->getData('telephone');
					$row['billing_company'] = $address->getData('company');
					$row['billing_fax'] = $address->getData('fax');
					
					$row['shipping_firstname'] = $address->getData('firstname');
					$row['shipping_lastname'] = $address->getData('lastname');
					$row['shipping_street_full'] = $address->getData('street');
					$row['shipping_street1'] = $address->getStreet(1);
					$row['shipping_street2'] = $address->getStreet(2);
					$row['shipping_city'] = $address->getData('city');
					$row['shipping_region'] = $address->getData('region');
					$row['shipping_country'] = $address->getData('country_id');
					$row['shipping_postcode'] = $address->getData('postcode');
					$row['shipping_telephone'] = $address->getData('telephone');
					$row['shipping_company'] = $address->getData('company');
					$row['shipping_fax'] = $address->getData('fax');
