...

    {
        ...
        try {
            ...
            if (isset($params['qty'])) {
                ...
                if($params['qty'] == 0 && $params['qty'] != 'undefined'){
                    
                    $ajaxData['status'] = 0;

                    $ajaxData['message'] = $this->__('a minimum quantity of 1 or more');

                    $this->getResponse()->setBody($callback.'('.Mage::helper('core')->jsonEncode($ajaxData).');');
            ...
            if(isset($params['type_product']) && $params['type_product'] ==1) {
                ...
                $this->getResponse()->setBody($callback.'('.Mage::helper('core')->jsonEncode($productInfo).');');
            ...
            if (!$this->_getSession()->getNoCartRedirect(true)) {
                ...
                if (!$cart->getQuote()->getHasError()){
                   ...
                   // $this->_getSession()->addSuccess($message);
                    ...
                    $pimage = Mage::helper('catalog/image')->init($product, 'small_image')->resize(55,83);
