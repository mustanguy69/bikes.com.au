...
class Amasty_Shopby_Block_Top extends Mage_Core_Block_Template
{
    ...
    protected function _isPageHandled($category)
    {
        ...
        if ($page->getUseCat()){
            ...
            $title = $titleSeparator . $page->getMetaTitle();
        ...
        if ($page->getTitle()) {
            ...
            $category->setData('meta_title', $page->getMetaTitle());

            $description2 = $page->getMetaDescr();
            define("AMASTY_META_DESCRIPTION", $description2);
            //$category->setData('meta_description', $page->getMetaDescr());
        }
    ...
    protected function _prepareLayout()
    {
        ...
        /*
         * Get and check how many filters are selectced
         */
        /* $appliedFilters = Mage::getSingleton('catalog/layer')->getState()->getFilters();
         $num_loops = "0";
         foreach ($appliedFilters as $item) {
             echo 'Name of the filter: '.$item->getName().'<br>'; // Name of the filter
             echo 'Currently selected value: '.$item->getLabel().'<br>'; // Currently selected value
             echo 'Filter code: '.$item->getFilter()->getRequestVar().'<br>'; // Filter code (usually attribute code, except category filter, where it equals "cat")
             echo $item->getCount()->getLabel().'<br>';
             $num_loops++;
         }
         echo 'Number Loops'.$num_loops;  */
    ...
    protected function changeCategoryData(Mage_Catalog_Model_Category $category)
    {
        ...
        $title2=$title.' | Bikes.com.au';
        //$category->setData('name', $title);

        $category->setData('name', $title);
        $category->setData('meta_title', $title2);
    ...
    protected function changeMetaData($head)
    {
        ...
        $oldTitle2 = '';
        //Main Category title
        $catTitle = Mage::getSingleton('catalog/layer')->getCurrentCategory()->getName();
        ...
        //$title = $this->insertContent($oldTitle2, $titles, $titlePosition, $titleSeparator);

        $final_title = $title.' | Bikes.com.au';
        $head->setTitle($final_title);
        ...
        $description2 = $title.' at Bikes.com.au.';
        define("AMASTY_META_DESCRIPTION", $description2);

        if(isset($replacedData["meta_description"]) && empty($replacedData["meta_title_from_url"])) {
            $head->setDescription($description, false, true);
        }else{
            //$description = $title.' '.$catTitle.' at Bikes.com.au.'; za sekoj slucaj moze ke zatreba $catTitle
            $description = $title.' at Bikes.com.au.';
            $head->setDescription($description, false, true);
        }
        ...
        //uncommnet to get keywords from Category
    ...
    protected function getOldMetaTitle($head)
    {
        ...
        //Shows all categories and sub-categories
        //$title = $head->getTitle();

        ///Shows only the last category
        //$title = $_category_detail->getName();

        $category_current_name = $_category_detail->getName();
        if($category_current_name == 'Root') {
            $title = $head->getTitle();
        }else{
            $title = $_category_detail->getName();
        }
