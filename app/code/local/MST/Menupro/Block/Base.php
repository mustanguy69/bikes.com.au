...
class MST_Menupro_Block_Base extends Mage_Core_Block_Template {
	...
	*/
	...
	public function menuDesignDir() {
		$packageName = Mage::getDesign()->getPackageName('frontend');
		$currentTheme = Mage::getSingleton('core/design_package')->getTheme('frontend');
		$path = Mage::getBaseDir('design') .DS."frontend".DS. $packageName .DS . $currentTheme . DS . "template".DS."menupro".DS."static".DS;
		if(!file_exists($path)) {
	...
	public function isGroupResponsive($style) {
		$allResponsiveStyle = Mage::getSingleton('menupro/groupmenu')->getMobileResponsiveStyles();
