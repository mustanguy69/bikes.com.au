...
class MST_Menupro_Block_Menu extends MST_Menupro_Block_Base {
	...
	public function getMenuData($menu, $permission) {
		...
		if ($this->_menuLink == Mage::helper ( 'menupro' )->getCurrentUrl () && $isDevelopMode) {
		...
		if ($menu->getType() != 6) {
			...
			if ($this->_aHref == "#") {
				if (!$isDevelopMode) {
					$this->_aText = '<span class="title"><?php echo $this->__("'. htmlentities($menuTitle, ENT_QUOTES, "UTF-8") .'") ?></span>';
				} else {
					$this->_aText = "<span class='title'>" . $this->__($menuTitle) . "</span>";
				}
			} else {
				if (!$isDevelopMode) {
					$this->_aText = '<span><?php echo $this->__("'. $this->__(htmlentities($menuTitle, ENT_QUOTES, "UTF-8")) .'") ?></span>';
	...
	public function getPushResponsiveHtml($groupId, $responsiveStyle) {
		return $this->getLayout()->createBlock('menupro/push')
	...
	public function getPushnewResponsiveHtml($groupId, $responsiveStyle) {
		return $this->getLayout()->createBlock('menupro/pushnew')
