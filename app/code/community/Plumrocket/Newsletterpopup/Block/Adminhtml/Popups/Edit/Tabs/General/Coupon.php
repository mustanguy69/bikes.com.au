    ...
    extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $couponCode = $popup->getCouponCode();
        $disabled = $couponCode == 0;
        $isAutogeneratedCoupon = false;

        if ($couponCode) {
            $rule = Mage::getModel('salesrule/rule')->load($couponCode);
            $isAutogeneratedCoupon = (bool)($rule && $rule->getUseAutoGeneration());
        }

        $form = new Varien_Data_Form();
        $form->setHtmlIdPrefix('popup_');

        $fieldset = $form->addFieldset('coupon_fieldset', array('legend' => Mage::helper('newsletterpopup')->__('Coupon Settings')));
        ...
        $popup->setData('just_label', Mage::helper('newsletterpopup')->__('Coupon code is not selected in the General section above. Coupon Settings section is disabled.'));
        ...
        $fieldset->addField('coupon_expiration_time', 'extended_time', array(
            'name'  => 'coupon_expiration_time',
            'label' => Mage::helper('newsletterpopup')->__('Coupon Expiration Time'),
            'note' => Mage::helper('newsletterpopup')->__('Set coupon dynamic expiration time. Select "00" in all fields for coupon code to never expire.'),
            'hidden' => ! $disabled,
            'disabled' => ! $isAutogeneratedCoupon,
        ));
