...
class Plumrocket_Newsletterpopup_Block_System_Config_Size extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    public function getElementHtml($element)
    {
        ...
        $html .= sprintf('<div style="width: 180px; float: left;">
        ...
        $html .= sprintf('<div style="width: 90px; float: left;"><input type="text" value="%s" name="%s" %s style="width:90px;"></div>',
            $value_height,
            $element->getName(),
            $element->serialize($element->getHtmlAttributes())
        );
