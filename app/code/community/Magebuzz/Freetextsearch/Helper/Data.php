...
class Magebuzz_Freetextsearch_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getSearchResultProducts($keyword){
		...
		/* Rudy added search by Search Synonym & Product ID */ 
		$products_list_productcode = $this->searchProductByAttribute($keyword,"productcode");
		if(count($products_list_productcode))
		{
			foreach($products_list_productcode as $productId)
			{
	...
	public function getSearchCMSPages($keyword){
		...
		->addFieldToFilter(array('title', 'content'), array(array('like'=>'%'.$keyword.'%'), array('like'=>'%'.$keyword.'%')))				
		/* Has been tweaked to search both title and content as above		
	...
	*/
	...
	{
		$temp = preg_replace("#[\s]+#", " ", $keyword);
		$multikeyword = explode(" ", $temp);
		...
		*/
		/* Hack to enable search to accept random keyword order */
		
		->addFieldToFilter($attribute,array('like'=>'%'.$multikeyword[0].'%'))	
		->addFieldToFilter($attribute,array('like'=>'%'.$multikeyword[1].'%'))	
		->addFieldToFilter($attribute,array('like'=>'%'.$multikeyword[2].'%'))	
		->addFieldToFilter($attribute,array('like'=>'%'.$multikeyword[3].'%'))	
		->addFieldToFilter($attribute,array('like'=>'%'.$multikeyword[4].'%'))
