	...
	public function quicksearchAction() {
		...
		if(count($productIds) > 0) {
			foreach($productIds as $productId) {
				...
				$html .= "<h3 class='product-name'><a class='product-img' href='".$product_url."' title='".$product_name."' target='_self'>".$product_name."</a></h3>";
		...
		if($allowSearchCMS) {
			...
			if(count($cmspageIds) > 0){
				foreach($cmspageIds as $_pageId){
					...
					$html .= "<h3 class='p-title'><a href='".$pageUrl."' title='".$pageTitle."' alt='".$pageTitle."' target='_self'>".$pageTitle."</a></h3>";
					$html .= "</li>";
				}
			}
		}
