		...
		if(count($results) > 0) {
			if(count($productIds) > 0) {
				...
				foreach($productIds as $productId) {
					...
					$img = $img->__toString();
					$price = $_coreHelper->currency($product->getFinalPrice(),true,false);
					$rrp = $_coreHelper->currency($product->getPrice(),true,false); /* An addition to show RRP */
					$product_url = $product->getProductUrl();
					$product_name = $product->getName();
					$product_desc = $product->getShortDescription();
					$html .= "<a class='product-img' href='".$product_url."' title='".$product_name."' alt='".$product_name."' target='_self'><img src='". $img ."' title='".$product_name."'></a>";
					$html .= "<div class='product-info'>";
					$html .= "<h3 class='product-name'><a href='".$product_url."' title='".$product_name."' target='_self'>".$product_name."</a></h3>";
					$html .= "<p class='desc'>".Mage::helper('freetextsearch')->prShortText($product_desc,$limitCharDesc)."</p>";
					/* Check if a product has RRP or not */
					if ((str_replace('$','',$rrp) - str_replace('$','',$price)) > 0) {
						$html .= "<div class='product-price'><h5>".$price."&nbsp;&nbsp;<span class='product-rrp-price'>".$rrp."</span></h5></div>";											
					} else {
						$html .= "<div class='product-price'><h5>".$price."</h5></div>";					
					}
					/* $html .= "<div class='product-price'><h5>".$price."&nbsp;&nbsp;<span class='product-rrp-price'>".$rrp."</span></h5></div>";	ORIGINAL*/
					$html .= "</div>";
					$html .= "</li>";
					$resultCount++;
			...
			if(count($productIds) < $limitResults || count($productIds) <= 0) {
				if($allowSearchCMS) {
					if(count($cmspageIds) > 0) {
						$html .= "<h3 class='result-title'>Web Pages</h3>";
						...
						foreach($cmspageIds as $_pageId){
							...
							$html .= "<h5 class='p-title'><a href='".$pageUrl."' title='".$pageTitle."' alt='".$pageTitle."' target='_self'>".$pageTitle."</a></h5>";
							$html .= "</li>";
							$resultCount++;
							if($resultCount >= $limitResults) {
								break;
