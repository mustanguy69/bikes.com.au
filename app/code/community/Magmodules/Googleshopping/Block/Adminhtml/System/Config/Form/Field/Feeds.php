    ...
    extends Mage_Adminhtml_Block_Abstract implements Varien_Data_Form_Element_Renderer_Interface
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        foreach ($storeIds as $storeId) {
            ...
            $htmlFeedlinks .= '<tr><td valign="top">' . $storeTitle . '<br/><small>Code: ' . $storeCode . '</small></td><td>' . $feedText . '</td><td><a href="' . $generateUrl . '">' . Mage::helper('googleshopping')->__('Generate New') . '</a><br/><a href="' . $downloadUrl . '">' . Mage::helper('googleshopping')->__('Download Last') . '</a></td></tr>';
        }

        if (empty($htmlFeedlinks)) {
            $htmlFeedlinks = Mage::helper('googleshopping')->__('No enabled feed(s) found');
        } else {
            $htmlHeader = '<div class="grid"><table cellpadding="0" cellspacing="0" class="border" style="width: 100%"><tbody><tr class="headings"><th>Store</th><th>Feed</th><th>Generate</th></tr>';
            $htmlFooter = '</tbody></table></div>';
            $htmlFeedlinks = $htmlHeader . $htmlFeedlinks . $htmlFooter;
        }

        return sprintf('<tr id="row_%s"><td colspan="6" class="label" style="margin-bottom: 10px;">%s</td></tr>', $element->getHtmlId(), $htmlFeedlinks);
    }
